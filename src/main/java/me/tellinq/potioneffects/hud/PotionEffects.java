...
public class PotionEffects extends BasicHud {
   ...
   * mod, this used to constantly be re-added every partial tick as the field used to make an array
   * list was made every partial tick as well.
   */
  @Subscribe
  ...
  protected void draw(UMatrixStack matrices, float x, float y, float scale, boolean example) {
    ...
    for (PotionEffect effect : this.currentEffects) {
      ...
      if (oComponent.duration) {
        ...
        if (showEffectDuringBlink(
            oBlinking, oBlinking.makeEffectDurationBlink, effect.getDuration(), example)) {
          switch (actualHorizontal) {
            case 0:
              timeX = xOffset;
              break;
   ...
   *     - If synced or if the example effects are running: Depending on the amount of counted ticks
   *     and the "speed" slider amount set, those two main factors determine if the element set
   *     should show while blinking. <br>
   *     - Depending on the effect's duration, that will determine if the effect should blink (will
   *     explain in depth later)
   */
  private boolean showEffectDuringBlink(
      EffectConfig effectConfig, boolean makeComponentBlink, float duration, boolean example) {
    if (effectConfig.blink && makeComponentBlink) {
      if (effectConfig.syncBlinking || (example && this.activeEffects.isEmpty())) {
        ...
        return duration / 20f > effectConfig.blinkDuration
            || duration % (50 - effectConfig.blinkSpeed) <= (50 - effectConfig.blinkSpeed) / 2f;
      }
    }
    return true;
  }
  ...
  public static class EffectConfig {
    ...
    @Checkbox(name = "Effect Name", description = "Show the effect name", subcategory = "Component")
    ...
    @Switch(
        name = "Sync Blinking",
        description =
            "Make blinking synced with tick counts or make blinking go based off the duration"
                + " time.",
        subcategory = "Blinking")
    ...
    @Color(name = "Name Color", description = "The color of the effect name", subcategory = "Color")
    ...
    @Dropdown(
        ...
        description =
            "Exclude effects that are either above, below, at, or not at a certain duration"
                + " threshold",
    ...
    @Dropdown(
        ...
        description =
            "Exclude effects that are either above, below, at, or not at a certain amplifier"
                + " amount",
        options = {"None", "Above", "Below", "At", "Not At"},
        subcategory = "Exclusion")
